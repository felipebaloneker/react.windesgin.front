  ...
  const createNewOrder = async (e: FormEvent) => {
    ...
    if (categoria !== undefined || details.length !== 0) {
      await Api.creatUserOrder(details, categoria)
        ...
        .catch(function () {
          return window.alert("Erro por favor tente novamente mais tarde.")
        })
      return window.alert(
        "Pedido Criado, clique em 'meus pedidos' para acompanhar"
      )
    } else {
      return window.alert(
        "Selecione uma categoria e detalhe melhor sua necessidade."
      )
    }
  }
        ...
        <form className="formulario" onSubmit={createNewOrder}>
          ...
          <div className="categoria_wrp">
            {listCategory.map((category) => {
              return (
                <div className="input_wrp" key={category.id}>
                  <input
                    type="radio"
                    id={category.name}
                    name="categoria"
          ...
          <div className="details">
            <label htmlFor="assunto">
              Decreva ainda melhor a sua necessidade:
            </label>
            <br />
            <textarea
              name=""
